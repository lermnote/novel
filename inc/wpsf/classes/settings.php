...
class WPSFramework_Settings extends WPSFramework_Abstract {
    ...
    public function validate_save($request) {
        ...
        if( isset ($request ['import']) && ! empty ($request ['import']) ) {
            ...
            $add_errors [] =  add_settings_error('success','success',esc_html__('Success. Imported backup options.', 'wpsf-framework'), 'updated');
        }

        if( isset ($request ['resetall']) ) {
            $add_errors [] =  add_settings_error('default options restored','default options restored',esc_html__('Default options restored.', 'wpsf-framework'), 'updated');
            return TRUE;
        }
        ...
        if( isset($request['reset']) && ! empty($section_id) ) {
            ...
            $add_errors[] =  add_settings_error('default options restored for only this section.','default options restored for only this section',esc_html__('Default options restored for only this section.', 'wpsf-framework'), 'updated');
        }
