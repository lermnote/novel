...
class Mail extends Theme_Abstract {
	...
	public function hooks() {
		add_action( 'phpmailer_init', [ $this, 'mail_smtp' ] );
		add_action( 'comment_unapproved_to_approved', [ $this, 'comment_approved' ] );
		add_action( 'comment_post', [ $this, 'comment_mail_notify' ] );
		add_action( 'wp_login_failed', [ $this, 'login_failed_notify' ] );
		add_action( 'delete_user', [ $this, 'delete_user_notify' ] );
		add_action( 'profile_update', [ $this, 'user_profile_update' ], 10, 2 );
		add_action( 'publish_post', [ $this, 'new_post_notify' ] );
	}
	...
	public function mail_header() {
		return '<div class="emailpaged" style="background-image: url(" ");-webkit-background-size: cover;-moz-background-size: cover;-o-background-size: cover;background-size: cover;background-position: center center;background-repeat: no-repeat;">
	...
	public function mail_footer() {
		return '<p style="color: #6e6e6e;font-size:13px;line-height:24px;">(此邮件由系统自动发出, 请勿回复。)</p>
			</div>
	...
	public function comment_approved( $comment ) {
		if ( is_email( $comment->comment_author_email ) ) {
			...
			$body      = $this->mail_header();
			$body     .= '留言审核通过通知</h1><div class="emailtext" style="background:#fff;padding:20px 32px 40px;"><p style="color: #6e6e6e;font-size:13px;line-height:24px;">您在' . get_option( 'blogname' ) . '《<a href="' . $post_link . '">' . get_the_title( $comment->comment_post_ID ) . '</a>》发表的评论：</p>
	...
	public function comment_mail_notify( $comment_id ) {
		...
		if ( ( '' != $parent_id ) && ( 'spam' != $spam_confirmed ) && ( $to != $admin_email ) ) {
			$wp_email = 'no-reply@' . preg_replace( '#^www\.#', '', strtolower( $_SERVER['SERVER_NAME'] ) );

			$subject = '您在 [' . get_option( 'blogname' ) . '] 的留言有了新回复';

			$message  = $this->mail_header();
			$message .= '您在' . get_option( 'blogname' ) . '上的留言有回复啦！</h1>
			...
			$message .= $this->mail_footer();

			$new  = 'Hey, ' . trim( get_comment( $parent_id )->comment_author ) . "\r\n\r\n";
            $new .= trim( $comment->comment_author ) . ' has replay your comment on 《' . get_the_title( $comment->comment_post_ID ) . '》:' . "\r\n";
			$new .= trim( get_comment( $parent_id )->comment_content ) . "\r\n";
			$new .= 'Reply as:' . "\r\n" .
			trim( $comment->comment_content ) . "\r\n";

			$from = 'From: "' . get_option( 'blogname' ) . "\" <$wp_email>";

			$headers = "$from\nContent-Type: multipart/alternative charset=" . get_option( 'blog_charset' ) . "\n";
			wp_mail( $to, $subject, $new, $headers );
		}
	}
	public function login_failed_notify() {
		date_default_timezone_set( 'PRC' );
		$admin_email     = get_bloginfo( 'admin_email' );
		$to              = $admin_email;
		$subject         = '【登录失败】有人使用了错误的用户名或密码登录' . get_bloginfo( 'name' ) . '！';
		$message         = $this->mail_header();
		$message         = get_bloginfo( 'name' ) . '账户登录失败通知！</h1>
		<div class="emailtext" style="background:#fff;padding:20px 32px 40px;">
	...
	public function user_profile_update( $user_id ) {
		...
		$message   = '您在' . $site_name . '账户资料修改成功！' . 'emailheaderbot ' . '<p style="color: #6e6e6e;font-size:13px;line-height:24px;">亲爱的 ' . $user_info->display_name . '<br/>您的资料修改成功!<br/>谢谢您的光临</p>';
	...
	public function new_post_notify( $post_ID ) {
